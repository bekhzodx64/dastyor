...
const DetailModal = ({modal, setModal, starRated, data}) => {
  ...
  return (
    <>
      {modal ? <div className='detail-page__review'>
            ...
            <div className='review__rating'>
              <div>
                {starRated.map((item, index) => (
                  <i
                    key={index}
                    onMouseEnter={() => setReview(state => {
                      return {
                        ...state,
                        rating: index + 1
                      }
                    })}
                    className={index < review.rating ? 'review__star active' : 'review__star'}>
                    {item}
                  </i>
                ))}
              </div>
